...
#region initialize MailMessage and recipients
        ...
        {
            foreach (var att in file_attachments.Value.Split(';'))
            {
                try
                {
                ...
                {
                    throw new Exception(string.Format("Unable to attach '{0}': {1}", att, e.Message));
                }
            }
        }
...
#region construct ICS file contents
        ...
        str.AppendLine(string.Format("METHOD:{0}", method.Value.ToUpper()));
        str.AppendLine(string.Format("SEQUENCE:{0}", sequence.Value));
        ...
        str.AppendLine(string.Format("CLASS:{0}", sensitivity.Value.ToUpper()));
        ...
        switch (mailPriority)
        {
            ...
            case MailPriority.Low:
                str.AppendLine("PRIORITY:9");
                break;
            case MailPriority.High:
                str.AppendLine("PRIORITY:1");
                break;
            default:
                break;
        }
        ...
        foreach (MailAddress addr in msg.To)
        {
            ...
            str.AppendLine(string.Format("ATTENDEE;CUTYPE=INDIVIDUAL;ROLE={3};{2};CN=\"{0}\";X-NUM-GUESTS=0:mailto:{1}", addr.DisplayName, addr.Address, rsvp_string, recipients_role.Value.ToUpper()));
        }

        foreach (MailAddress addr in msg.CC)
        {
            if (addr.Address == msg.From.Address) organizer_in_recipients = true;
            str.AppendLine(string.Format("ATTENDEE;CUTYPE=INDIVIDUAL;ROLE={3};{2};CN=\"{0}\";X-NUM-GUESTS=0:mailto:{1}", addr.DisplayName, addr.Address, rsvp_string, copy_recipients_role.Value.ToUpper()));
        }
